...
app.post('/api/orders', async (req, res) => {
    try {
        ...
        if (!restaurant)
            throw new Error("No restaurant initialized");
...
app.post('/api/webhooks/aba', async (req, res) => {
...
app.post('/api/pms/folio', async (req, res) => {
    try {
        ...
        if (order.status !== 'DELIVERED')
            return res.status(400).json({ error: 'Order must be delivered before posting to PMS' });
